...
const int lightbulb = 25; // GIOP22 pin connected to TRIG pin of ultrasonic sensor 2
...
const char* lightintensity1 = "smarthome/devices/ldr1";
const char* lightintensity2 = "smarthome/devices/ldr2";
...
void TaskHCIControl( void *pvParameters )
{
  ...
  { limitSwitch.loop(); // MUST call the loop() function first
    ...
    //
    //      Serial.println("Blind State HCI: ");
    //    Serial.println(blindstatehci);
    //
    //    Serial.println("Light State HCI: ");
    ...
    if (overstatus == 0) {
      ...
      //raise blinds
        ...
        int i = 0;
        state = limitSwitch.getState();
        Serial.println("open blinds state: ");
        Serial.println(state);
        while (i < 10) {
          Serial.println("HCI");
          limitSwitch.loop();
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect(clientID.c_str())) {
      ...
      client.subscribe(lightintensity1);
      client.subscribe(lightintensity2);
...
void callback(char* topic, byte * payload, unsigned int length) {
  ...
  //Sensor Control
  else if (strcmp(topic, lightintensity1) == 0) {
    if ( incomingMessage.toFloat() < 10000) {
      //raise blinds and turn off light
      blindstatesensor = 1;
      lightstatesensor = 1;

    }
    else if (incomingMessage.toFloat() >= 10000) {
